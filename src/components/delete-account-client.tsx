...
export function DeleteAccountClient({ token }: Props) {
	const [result, setResult] = useState<ResultState>({ type: "loading", message: "Processing your request..." });

	useEffect(() => {
		fetch(`/_api/account/deletion-confirm?token=${encodeURIComponent(token)}`)
			.then((res) => {
				if (res.ok) {
					return res.json();
				}
			...
			.then(() => {
				setResult({ type: "success", message: "Your account has been deleted successfully." });
				return;
			})
			.catch((err: Error) => {
				setResult({
	...
	if (result.type === "loading") {
		return (
			<>
				...
				<h1 className="text-3xl font-semibold tracking-tight">Deleting account</h1>
	...
	if (result.type === "success") {
		return (
			<>
				...
				<h1 className="text-3xl font-semibold tracking-tight">Account deleted</h1>
				<p className="text-muted-foreground mt-4 text-sm">{result.message}</p>
